...
public class FirmwareUpdateActivity extends Activity implements View.OnClickListener, AdapterView.OnItemSelectedListener, FirmwareUpdateListener {
    ...
    private void updateFirmware() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    mPrinter.updateFirmware(targetFirmwareInfo, secureOption, mFirmwareUpdateListener, mContext);
                } catch (Exception e) {
                    mProgressIndicator.endProgress();
    ...
    private void verifyUpdate(final FirmwareInfo targetFirmwareInfo) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    mPrinter.verifyUpdate(targetFirmwareInfo, secureOption, mFirmwareUpdateListener);
                } catch (Exception e) {
                    mProgressIndicator.endProgress();
    ...
    public void onUpdateFirmware(int code, int maxWaitTime) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                // Reconect printer with 3 sec interval until maxWaitTime sec done.
                boolean ret = RebootWaitingWorker.reconnect(mPrinter, maxWaitTime, 3);
                if(!ret) {
                    mProgressIndicator.endProgress();
                    return;
                }
    ...
    private void updateWaitingMessage(final String msg) {
        ...
        mProgressIndicator.changeProgress(progressMsg);
    }
