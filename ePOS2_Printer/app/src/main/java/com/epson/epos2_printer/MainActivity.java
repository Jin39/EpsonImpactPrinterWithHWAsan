...
public class MainActivity extends Activity implements View.OnClickListener, ReceiveListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int[] target = {
            ...
            R.id.btnSampleCoupon,
                R.id.btnMonitor,
            R.id.btnSetting,
            R.id.btnHwasanStatus,
            R.id.btnSafeTest,
            R.id.btnBufferOverflow,
            R.id.btnUseAfterFree,
            R.id.btnDoubleFree
        };
        ...
        seriesAdapter.add(new SpnModelsItem(getString(R.string.printerseries_u220ii), Printer.TM_U220II));
        seriesAdapter.add(new SpnModelsItem(getString(R.string.printerseries_sbh50), Printer.SB_H50));
    ...
    protected void onActivityResult(int requestCode, final int resultCode, final Intent data) {
        if (data != null && resultCode == RESULT_OK) {
            ...
            mIp = data.getStringExtra(getString(R.string.ipAddress));
        }
    }
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.btnHwasanStatus:
                runHWASanTest("status");
                break;

            case R.id.btnSafeTest:
                runHWASanTest("safe");
                break;

            case R.id.btnBufferOverflow:
                runHWASanTest("buffer_overflow");
                break;

            case R.id.btnUseAfterFree:
                runHWASanTest("use_after_free");
                break;

            case R.id.btnDoubleFree:
                runHWASanTest("double_free");
                break;
    ...
    // HWASan Test Methods
    private void runHWASanTest(String testType) {
        new Thread(new Runnable() {
            public void run() {
                try {
                    HWASanTestHelper testHelper = new HWASanTestHelper();
                    String result = "";

                    switch (testType) {
                        case "status":
                            result = testHelper.getHWASanStatus();
                            break;
                        case "safe":
                            result = testHelper.testSafeOperation();
                            break;
                        case "buffer_overflow":
                            result = testHelper.testBufferOverflow();
                            break;
                        case "use_after_free":
                            result = testHelper.testUseAfterFree();
                            break;
                        case "double_free":
                            result = testHelper.testDoubleFree();
                            break;
                        default:
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            // ShowMsg.showResult(finalResult, mContext);
                        }
                    });
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            ShowMsg.showMsg("HWASan library not loaded. Make sure to build with 'hwasan' build type.", mContext);
                        }
                    });
                } catch (Exception e) {
                    final String error = "HWASan test error: " + e.getMessage();
                    runOnUiThread(new Runnable() {
                        public void run() {
                            ShowMsg.showMsg(error, mContext);
                        }
                    });
                }
            }
